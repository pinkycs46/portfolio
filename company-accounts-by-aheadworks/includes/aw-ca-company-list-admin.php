...
class AwcaCompanyListAdmin extends WP_List_Table {
	...
	public function process_bulk_action() {
		...
		if ('approved' === $action || 'blocked' === $action || 'declined' === $action) {
			...
			if (!empty($ids)) {
				...
				switch ($action) {
					case 'approved': 
						$template = 'New Company Approved';
						break;
					case 'blocked': 
						$template = 'Company account blocked';
						break;
					case 'declined': 
						$template = 'Company account rejected';
						break;
				}
				if (is_array($ids)) {
					foreach ($ids as $id) {
						$record = aw_ca_get_company_by_id($id);
						$company_admin_id = $record->company_admin_id;
						aw_ca_send_mail_notification( $company_admin_id, 'New Company Approved');
					}
				} else {
						$record = aw_ca_get_company_by_id($ids);
						$company_admin_id = $record->company_admin_id;

						aw_ca_send_mail_notification( $company_admin_id, $template);
				}
			}
		}
	}
	...
	public function get_count( $user_id = '', $search = '') {
		...
		if ('' != $search) {
			...
			$total_items = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}aw_ca_company_information WHERE (`company_name` LIKE %s OR `country` LIKE %s OR `city` LIKE %s OR `sales_representative` LIKE %s OR `customer_group` LIKE %s)", "%{$search}%", "%{$search}%", "%{$search}%", "%{$search}%", "%{$search}%"));
